...
class PlanningGraph():
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        add_a1 = set(node_a1.action.effect_add)
        add_a2 = set(node_a2.action.effect_add)

        rem_a1 = set(node_a1.action.effect_rem)
        rem_a2 = set(node_a2.action.effect_rem)
        ...
        for goal in self.problem.goal:
            ...
            for level in range(len(self.s_levels)):
                for state in self.s_levels[level]:
                    ...
                    state_literal = expr(state.symbol)
                    if not state.is_pos:
                        state_literal = expr('~{}'.format(state.symbol))

                    if goal == state_literal:
                        flag = True
                        level_sum += level
                        break
                if flag:
